...
struct fifo_entry {
	void *fifo_reserved;
	uint8_t data[sizeof(struct reader_data)];
};

int aggregator_put(struct reader_data in_data)
{
	...
	memcpy(fifo_data->data, &in_data, sizeof(struct reader_data));
...
int aggregator_get(struct reader_data *out_data)
{
	void  *fifo_data;
	int   err = 0;

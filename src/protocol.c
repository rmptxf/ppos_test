		      ...
		      const struct mqtt_evt *evt)
{
	switch (evt->type) {
        ...
        {
            LOG_INF("mqtt client disconnected err %d!", evt->result);
            connected = false;
        } break;
        ...
        case MQTT_EVT_PUBLISH: 
        {
            ...
            LOG_INF("   id: %d, qos: %d", pub->message_id, pub->message.topic.qos);
            LOG_INF("   item: %s", log_strdup(pub->message.topic.topic.utf8));
...
init:
	while (retries) {
		...
		hints.ai_protocol = 0;
		cnt = 0;
		while ((err = getaddrinfo(hostname, NULL, &hints, &haddr)) && cnt < 3) {
			LOG_ERR("unable to get address for broker, retrying ..");
			cnt++;
		}

		if (err != 0) {
			LOG_ERR("unable to get address for broker, err %d!", err);
			return;
		}
		LOG_INF("host dns resolved!");
		...
		if (err != 0) {
			LOG_ERR("unable to generate jwt on modem, err %d!", err);
			return;
		}
		...
		if (err != 0) {
			LOG_ERR("could not connect, err %d!", err);
	...
	while (1) {
		...
		if (err) {
			LOG_ERR("could not publish, err %d!", err);
		}
